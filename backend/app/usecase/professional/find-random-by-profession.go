...
func (uc *FindRandomUC) Execute() ([]*ProfessionalPresenter, int64, error) {
	var namePtr *string
	var premiums []*ProfessionalPresenter
	var regulars []*ProfessionalPresenter
	...
	for _, p := range pros {
		...
		if pp.IsPremium != nil && *p.IsPremium {
			premiums = append(premiums, &pp)
		} else {
			regulars = append(regulars, &pp)
		}
	}
	presenters = append(premiums, regulars...)
