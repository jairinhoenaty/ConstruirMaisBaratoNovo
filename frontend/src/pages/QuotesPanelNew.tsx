...
function QuotesPanel() {
  ...
  React.useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (response.status == 200) {
        ...
        if (profile == "profissional" && json?.length > 0) {
          const quotesWithAccess = await Promise.all(
            json.map(async (quote: IBudget) => {
              try {
                const accessResponse = await BudgetService.checkBudgetAccess(quote.id);
                return {
                  ...quote,
                  budgetAccess: accessResponse.data,
                };
              } catch (error) {
                console.error(`Erro ao verificar acesso do orçamento ${quote.id}:`, error);
                return {
                  ...quote,
                  budgetAccess: {
  ...
  const handleStatusChange = async (budget_approved: IBudget, approved: boolean) => {
      ...
      <>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-700">Dados do Cliente</h2>
            {!hasAccess && (
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  {hasAccess ? (
                    <p className="text-gray-900 font-medium">{message.telephone}</p>
                  ) : (
  ...
  const renderProfessionalInfo = (message: Quote) => {
    ...
    return (
      <>
        <div className="border-t border-gray-100 pt-4"></div>
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-gray-700">Dados do Profissional</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <span className="block text-sm text-gray-500">Nome</span>
                  <p className="text-gray-900 font-medium">{message.professionals?.[0]?.name}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <span className="block text-sm text-gray-500">Email:</span>
                  <p className="text-gray-900 font-medium">{message.professionals?.[0]?.email}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <span className="block text-sm text-gray-500">WhatsApp:</span>
                  <p className="text-gray-900 font-medium">{message.professionals?.[0]?.telephone}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                <MapPin className="w-5 h-5 mt-1 text-gray-400" />
                <div>
                  <span className="block text-sm text-gray-500">UF/Cidade:</span>
                  <p className="text-gray-900 font-medium">
            ...
            <div className="md:col-span-2 flex flex-wrap items-center gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {message.professionals?.[0]?.professions?.map((element: any, index: number) => (
                  <span
                    key={index}
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-8">
      ...
      <div className="space-y-6">
        {quotes.map((quote) => {
          const profession = professionals.find((p) => p.id === quote.profession);
          const state = states.find((s) => s.id === quote.state);
          const city = citiesByState[quote.state]?.find((c) => c.id === quote.city);
          ...
          return (
            ...
            >
                  ...
                  {profile == "admin" && (
                    <label className="inline-flex items-center cursor-pointer">
                      <input
                        ...
                        onChange={() => handleStatusChange(quote, quote.approved)}
                      />
                      <span className="ml-2 text-sm text-gray-700">Aprovar</span>
                    </label>
                  )}
                  <button
              ...
              {(profile == "admin" || profile == "client") && renderProfessionalInfo(quote)}
              {(profile == "admin" || profile == "profissional") && renderClientInfoPreview(quote)}
