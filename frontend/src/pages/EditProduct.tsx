...
interface prodEditProduto {
  id: number;
}

function EditProduct({ id }: prodEditProduto) {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await ProductService.productById(id);
      const json = await response.data;
      if (response.status == 200) {
        // localStorage.setItem("post_id", response.data.oid);
        //formData.title = json.name;
        setFormData((prev) => ({
          ...prev,
          title: json.name,
          photo: json.image ?? "",
          ...
          //discount: 0,
          approved: false,
          dayoffer: json.dayoffer,
          professionID: json.category.ProfessionID,
          mainCategory: json.category.ProfessionID,
          categoryID: json.categoryID,
          subcategory: json.categoryID,
